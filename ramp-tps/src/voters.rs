...
fn delegate_stake(
    ...
    loop {
        ...
        let instructions = stake_instruction::create_account_and_delegate_stake(
            ...
            &Lockup::default(),
            nzt_to_lamports(nzt_gift as f64),
        );
...
pub fn award_stake(
    ...
    for (node_pubkey, vote_account_pubkey) in voters {
        info!("Delegate {} NZT to {}", nzt_gift, node_pubkey);
        delegate_stake(rpc_client, faucet_keypair, vote_account_pubkey, nzt_gift);
        buffer.push(format!("Delegated {} NZT to {}", nzt_gift, node_pubkey));
    }
