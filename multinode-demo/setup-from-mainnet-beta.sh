...
rm -rf "$NZT_CONFIG_DIR"/latest-mainnet-beta-snapshot
mkdir -p "$NZT_CONFIG_DIR"/latest-mainnet-beta-snapshot
(
  cd "$NZT_CONFIG_DIR"/latest-mainnet-beta-snapshot || exit 1
  set -x
...
snapshot=$(ls "$NZT_CONFIG_DIR"/latest-mainnet-beta-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$exzo_ledger_tool create-snapshot \
  --ledger "$NZT_CONFIG_DIR"/latest-mainnet-beta-snapshot \
