...
if [[ -f $BOOTSTRAP_VALIDATOR_STAKE_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_STAKE_KEYPAIR" "$NZT_CONFIG_DIR"/bootstrap-validator/stake-account.json
else
...
if [[ -f $BOOTSTRAP_VALIDATOR_VOTE_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_VOTE_KEYPAIR" "$NZT_CONFIG_DIR"/bootstrap-validator/vote-account.json
else
...
"$NZT_ROOT"/fetch-spl.sh
if [[ -r spl-genesis-args.sh ]]; then
    SPL_GENESIS_ARGS=$(cat "$NZT_ROOT"/spl-genesis-args.sh)
    #shellcheck disable=SC2207
    #shellcheck disable=SC2206
    args+=($SPL_GENESIS_ARGS)
fi

default_arg --ledger "$NZT_CONFIG_DIR"/bootstrap-validator
default_arg --faucet-pubkey "$NZT_CONFIG_DIR"/faucet.json
