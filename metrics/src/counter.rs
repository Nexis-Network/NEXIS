...
impl Counter {
    fn default_metrics_rate() -> u64 {
        let v = env::var("NZT_DEFAULT_METRICS_RATE")
    ...
    fn default_log_rate() -> usize {
        let v = env::var("NZT_DEFAULT_LOG_RATE")
...
mod tests {
    ...
    fn test_metricsrate() {
        try_init_logger_at_level_info().ok();
        let _readlock = get_env_lock().read();
        env::remove_var("NZT_DEFAULT_METRICS_RATE");
    ...
    fn test_metricsrate_env() {
        ...
        env::set_var("NZT_DEFAULT_METRICS_RATE", "50");
        static mut COUNTER: Counter = create_counter!("test", 1000, 0);
        unsafe {
            COUNTER.init();
    ...
    fn test_lograte() {
        ...
        assert_eq!(
            ...
            DEFAULT_LOG_RATE,
            "default_log_rate() is {}, expected {}, NZT_DEFAULT_LOG_RATE environment variable set?",
            Counter::default_log_rate(),
            DEFAULT_LOG_RATE,
        );
    ...
    fn test_lograte_env() {
        ...
        env::set_var("NZT_DEFAULT_LOG_RATE", "50");
        unsafe {
            COUNTER.init();
        ...
        env::set_var("NZT_DEFAULT_LOG_RATE", "0");
        unsafe {
            COUNTER2.init();
