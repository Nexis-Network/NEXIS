...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = nexis_perf::perf_libs::locate_perf_libs()
            {
                nexis_perf::perf_libs::append_to_ld_library_path(
...
mod tests {
    use {
        super::*,
        nexis_perf::test_tx::{test_invalid_tx, test_tx},
        nexis_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
