...
pub use {
    nexis_gossip::cluster_info::MINIMUM_VALIDATOR_PORT_RANGE_WIDTH,
    nexis_test_validator as test_validator,
};
...
pub fn redirect_stderr_to_file(logfile: Option<String>) -> Option<JoinHandle<()>> {
    ...
    let filter = "nexis=info";
    match logfile {
        None => {
            nexis_logger::setup_with_default(filter);
            None
        }
        Some(logfile) => {
            #[cfg(unix)]
            ...
            #[cfg(not(unix))]
            {
                ...
                nexis_logger::setup_file_with_default(&logfile, filter);
                None
            }
        }
    }
...
pub fn port_range_validator(port_range: String) -> Result<(), String> {
    if let Some((start, end)) = nexis_net_utils::parse_port_range(&port_range) {
