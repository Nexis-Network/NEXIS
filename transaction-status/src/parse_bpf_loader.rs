...
mod test {
    use {
        super::*,
        serde_json::Value,
        nexis_sdk::{message::Message, pubkey},
    };
    ...
    fn test_parse_bpf_loader_instructions() {
        ...
        let instruction = nexis_sdk::loader_instruction::write(
            &account_pubkey,
            &program_id,
            offset,
            bytes.clone(),
        ...
        let instruction = nexis_sdk::loader_instruction::finalize(&account_pubkey, &program_id);
        let message = Message::new(&[instruction], Some(&fee_payer));
        assert_eq!(
            parse_bpf_loader(&message.instructions[0], &account_keys).unwrap(),
            ParsedInstructionEnum {
    ...
    fn test_parse_bpf_upgradeable_loader_instructions() {
        ...
        let instructions = nexis_sdk::bpf_loader_upgradeable::create_buffer(
            &keys[0],
            &keys[1],
            &keys[2],
            55,
        ...
        let instruction =
            nexis_sdk::bpf_loader_upgradeable::write(&keys[1], &keys[0], offset, bytes.clone());
        ...
        let instructions = nexis_sdk::bpf_loader_upgradeable::deploy_with_max_program_len(
            &keys[0],
            &keys[1],
            &keys[4],
            &keys[2],
        ...
        let instruction =
            nexis_sdk::bpf_loader_upgradeable::upgrade(&keys[2], &keys[3], &keys[0], &keys[4]);
        let message = Message::new(&[instruction], None);
        assert_eq!(
        ...
        let instruction =
            nexis_sdk::bpf_loader_upgradeable::set_buffer_authority(&keys[1], &keys[0], &keys[2]);
        let message = Message::new(&[instruction], None);
        assert_eq!(
        ...
        let instruction = nexis_sdk::bpf_loader_upgradeable::set_upgrade_authority(
            &keys[1],
            &keys[0],
            Some(&keys[2]),
        );
        ...
        let instruction =
            nexis_sdk::bpf_loader_upgradeable::set_upgrade_authority(&keys[1], &keys[0], None);
        let message = Message::new(&[instruction], None);
        assert_eq!(
            parse_bpf_upgradeable_loader(&message.instructions[0], &keys[0..2]).unwrap(),
            ParsedInstructionEnum {
        ...
        let instruction = nexis_sdk::bpf_loader_upgradeable::close(&keys[0], &keys[1], &keys[2]);
        let message = Message::new(&[instruction], None);
        assert_eq!(
