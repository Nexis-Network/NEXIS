## Overview
`nexis-stake-monitor` is a utility that scans all transactions to ensure that stake accounts remain in compliance with the following rules:
...
In terms of `nexis` command-line subcommands:
...
System accounts can also be manually enrolled with the `nexis-stake-monitor enroll` subcommand.
