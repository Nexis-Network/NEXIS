...
use console::Emoji;
use log::*;
use nexis_clap_utils::{
    input_parsers::pubkey_of,
    input_validators::{is_pubkey, is_slot, is_url},
};
use nexis_client::rpc_client::RpcClient;
use nexis_metrics::datapoint_error;
use nexis_sdk::{clock::Slot, native_token::lamports_to_sol, pubkey::Pubkey, system_program};
use nexis_stake_monitor::*;
use std::{fs, io, process};

fn load_accounts_info(data_file: &str) -> AccountsInfo {
    let data_file_new = data_file.to_owned() + "new";
    let accounts_info = nexis_cli_config::load_config_file(&data_file_new)
        .or_else(|_| nexis_cli_config::load_config_file(data_file))
        .unwrap_or_default();
...
fn save_accounts_info(data_file: &str, accounts_info: &AccountsInfo) -> io::Result<()> {
    let data_file_new = data_file.to_owned() + "new";
    nexis_cli_config::save_config_file(&accounts_info, &data_file_new)?;
...
fn command_enroll(data_file: &str, json_rpc_url: String, account_address: &Pubkey) {
    ...
    println!(
        "Enrolled {} at slot {} with a balance of {} NZT",
        account_address,
        slot,
        lamports_to_nzt(account.lamports)
    );
}
...
fn command_check(data_file: &str, account_address: &Pubkey) {
    ...
    if let Some(account_info) = accounts_info.account_info.get(&account_address.to_string()) {
        if let Some(slot) = account_info.compliant_since {
            println!(
                "{}Account compliant since slot {} with a balance of {} NZT",
                Emoji("✅ ", ""),
                slot,
                lamports_to_nzt(account_info.lamports)
            );
            process::exit(0);
        } else {
            eprintln!(
...
fn main() {
    ...
    nexis_metrics::set_panic_hook("stake-monitor");
