//! This module contains the definition of various staker information and the function to add them to the genesis configuration.
//!
//! The staker information includes the name, staker address, amount of lamports, and optional withdrawer address.
//! The module also defines different unlock information structures for different lockup schedules.
//!
//! The `add_genesis_accounts` function takes a mutable reference to the `GenesisConfig` and an initial amount of issued lamports.
//! It adds the staker information to the genesis configuration by calling the `add_stakes` function for each category of stakers.
//! The `add_stakes` function calculates and adds the stakes for a given category of stakers based on the provided unlock information.
//!
//! The `add_genesis_accounts` function is primarily used to initialize the genesis configuration with the specified staker information and lockup schedules.
//!
//! # Example
//!
//! ```
//! use nexis_sdk::genesis_config::GenesisConfig;
//! use nexis_genesis_accounts::{add_genesis_accounts, UNLOCKS_HALF_AT_9_MONTHS};
//!
//! let mut genesis_config = GenesisConfig::default();
//! let issued_lamports = 0;
//!
//! add_genesis_accounts(&mut genesis_config, issued_lamports);
//!
//! // Verify the total amount of lamports in the genesis configuration
...
//!     .sum::<u64>();
//!
//! assert_eq!(500_000_000 * LAMPORTS_PER_NZT, lamports);
//! ```
//!
//! For more information, refer to the [Nexis SDK documentation](https://docs.nexis.com/sdk).
use {
    crate::{
        stakes::{create_and_add_stakes, StakerInfo},
        unlocks::UnlockInfo,
    },
    nexis_sdk::{genesis_config::GenesisConfig, native_token::LAMPORTS_PER_NZT},
};
...
pub const CREATOR_STAKER_INFOS: &[StakerInfo] = &[
    StakerInfo {
        ...
        lamports: 5_000_000 * LAMPORTS_PER_NZT,
    ...
    StakerInfo {
        ...
        lamports: 1_205_602 * LAMPORTS_PER_NZT,
    ...
    StakerInfo {
        ...
        lamports: 3_655_292 * LAMPORTS_PER_NZT,
    ...
    StakerInfo {
        ...
        lamports: 103_519 * LAMPORTS_PER_NZT,
    ...
    StakerInfo {
        ...
        lamports: 3_880_295 * LAMPORTS_PER_NZT,
...
pub const SERVICE_STAKER_INFOS: &[StakerInfo] = &[
    StakerInfo {
        ...
        lamports: 225_000 * LAMPORTS_PER_NZT,
...
pub const FOUNDATION_STAKER_INFOS: &[StakerInfo] = &[
    ...
    StakerInfo {
        ...
        lamports: 57_500_000 * LAMPORTS_PER_NZT,
...
pub const GRANTS_STAKER_INFOS: &[StakerInfo] = &[
    ...
    StakerInfo {
        ...
        lamports: 15_000_000 * LAMPORTS_PER_NZT,
...
pub const COMMUNITY_STAKER_INFOS: &[StakerInfo] = &[
    ...
    StakerInfo {
        ...
        lamports: 30_301_032 * LAMPORTS_PER_NZT,
    ...
    StakerInfo {
        ...
        lamports: 153_333_633 * LAMPORTS_PER_NZT + 41 * LAMPORTS_PER_NZT / 100,
...
pub fn add_genesis_accounts(genesis_config: &mut GenesisConfig, mut issued_lamports: u64) {
    ...
    create_and_add_stakes(
        ...
        &StakerInfo {
            ...
            lamports: (500_000_000 * LAMPORTS_PER_NZT).saturating_sub(issued_lamports),
