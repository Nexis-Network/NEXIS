...
```bash
nexis-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
nexis-tokens transaction-log --output-path transactions.csv
```
...
```bash
nexis-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
nexis-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
nexis-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
    --input-csv <ALLOCATIONS_CSV> \
    --stake-authority <KEYPAIR> --withdraw-authority <KEYPAIR> --fee-payer <KEYPAIR>
```

Currently, this will subtract 1 NZT from each allocation and store it the
recipient address. That NZT can be used to pay transaction fees on staking
...
```bash
nexis-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
nexis-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `nexis-tokens balances`.
...
```bash
nexis-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
