...
impl TryFrom<&[u8]> for NexisAccountType {
    type Error = ParseError;

    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        if data.len() == ACCOUNT_LEN {
            VAccountInfo::try_from_slice(data).map(NexisAccountType::Account)
        } else {
            VAccountStorage::deserialize_stream_array(data).map(NexisAccountType::Storage)
        }
