/**
 * @brief Example C-based BPF program uses nzt_log_data
 */
#include <nexis_sdk.h>
...
extern uint64_t entrypoint(const uint8_t *input) {
  ...
  fields[0].len = nzt_strlen((char*)fields[0].addr);
  fields[1].addr = fields[0].addr + fields[0].len + 1;
  fields[1].len = params.data_len - fields[0].len - 1;

  nzt_set_return_data(return_data, sizeof(return_data));

  nzt_log_data(fields, 2);
