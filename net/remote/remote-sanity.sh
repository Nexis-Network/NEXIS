...
local|tar|skip)
  ...
  nexis_cli=nexis
  nexis_gossip=nexis-gossip
  nexis_install=nexis-install
  ;;
*)
...
(
  set -x
  $nexis_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $nexis_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp:nexis-install test"
