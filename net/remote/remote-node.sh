...
cat >> ~/nexis/on-reboot <<EOF
  ...
  sudo RUST_LOG=info ~nexis/.cargo/bin/nexis-sys-tuner --user $(whoami) > sys-tuner.log 2>&1 &
  echo \$! > sys-tuner.pid
...
EOF
  ...
  bootstrap-validator)
    ...
    if [[ $skipSetup != true ]]; then
      clear_config_dir "$NZT_CONFIG_DIR"
      ...
      setupValidatorKeypair() {
        ...
        if [[ -f net/keypairs/"$name".json ]]; then
         ...
         nexis-keygen new --no-passphrase -so config/"$name".json
          if [[ "$name" =~ ^validator-identity- ]]; then
            name="${name//-identity-/-vote-}"
           nexis-keygen new --no-passphrase -so config/"$name".json
            name="${name//-vote-/-stake-}"
        ...
        if [[ -n $internalNodesLamports ]]; then
          declare pubkey
          pubkey="$(nexis-keygen pubkey config/"$name".json)"
...
EOF
      ...
      for i in $(seq 0 $((numBenchTpsClients-1))); do
       ...
       nexis-bench-tps --write-client-keys config/bench-tps"$i".yml \
      ...
      if [[ "$extraPrimordialStakes" -gt 0 ]]; then
        ...
        for i in $(seq "$extraPrimordialStakes"); do
          args+=(--bootstrap-validator "$(nexis-keygen pubkey "config/validator-identity-$i.json")"
                                       "$(nexis-keygen pubkey "config/validator-vote-$i.json")"
                                       "$(nexis-keygen pubkey "config/validator-stake-$i.json")"
          )
        done
      fi
      ...
      if [[ -n "$maybeWarpSlot" ]]; then
       ...
       nexis-ledger-tool -l config/bootstrap-validator create-snapshot 0 config/bootstrap-validator $maybeWarpSlot
      fi

     nexis-ledger-tool -l config/bootstrap-validator shred-version --max-genesis-archive-unpacked-size 1073741824 | tee config/shred-version

      if [[ -n "$maybeWaitForSupermajority" ]]; then
        bankHash=$(nexis-ledger-tool -l config/bootstrap-validator bank-hash)
    ...
    if [[ "$tmpfsAccounts" = "true" ]]; then
      args+=(--accounts /mnt/nexis-accounts)
    fi
...
EOF
  ...
  validator|blockstreamer)
    ...
    if [[ $skipSetup != true ]]; then
      ...
      if [[ $nodeType = blockstreamer ]]; then
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/nexis/config/blockstreamer-identity.json "$NZT_CONFIG_DIR"/validator-identity.json
      else
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/nexis/config/validator-identity-"$nodeIndex".json "$NZT_CONFIG_DIR"/validator-identity.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/nexis/config/validator-stake-"$nodeIndex".json "$NZT_CONFIG_DIR"/stake-account.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/nexis/config/validator-vote-"$nodeIndex".json "$NZT_CONFIG_DIR"/vote-account.json
      fi
      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/nexis/config/shred-version "$NZT_CONFIG_DIR"/shred-version

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/nexis/config/bank-hash "$NZT_CONFIG_DIR"/bank-hash || true

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/nexis/config/faucet.json "$NZT_CONFIG_DIR"/faucet.json
    fi
    ...
    args=(
      ...
      --gossip-port 8001
      --rpc-port 8899
      --expected-shred-version "$(cat "$NZT_CONFIG_DIR"/shred-version)"
    )
    if [[ $nodeType = blockstreamer ]]; then
      args+=(
        --blockstream /tmp/nexis-blockstream.sock
        --no-voting
        --dev-no-sigverify
    ...
    if [[ ! -f "$NZT_CONFIG_DIR"/validator-identity.json ]]; then
     nexis-keygen new --no-passphrase -so "$NZT_CONFIG_DIR"/validator-identity.json
    fi
    args+=(--identity "$NZT_CONFIG_DIR"/validator-identity.json)
    if [[ ! -f "$NZT_CONFIG_DIR"/vote-account.json ]]; then
     nexis-keygen new --no-passphrase -so "$NZT_CONFIG_DIR"/vote-account.json
    fi
    args+=(--vote-account "$NZT_CONFIG_DIR"/vote-account.json)
    ...
    if [[ -r "$NZT_CONFIG_DIR"/bank-hash ]]; then
      args+=(--expected-bank-hash "$(cat "$NZT_CONFIG_DIR"/bank-hash)")
    fi
    ...
    scp "$entrypointIp":~/nexis/config/faucet.json "$NZT_CONFIG_DIR"/
    if [[ $nodeType = blockstreamer ]]; then
...
EOF
    ...
    if [[ $skipSetup != true && $nodeType != blockstreamer && -z $maybeSkipAccountsCreation ]]; then
      ...
      nexis--url http://"$entrypointIp":8899 catchup config/validator-identity.json
      ...
      if [[ ${extraPrimordialStakes} -eq 0 ]]; then
        ...
        multinode-demo/delegate-stake.sh --vote-account "$NZT_CONFIG_DIR"/vote-account.json \
                                         --stake-account "$NZT_CONFIG_DIR"/stake-account.json \
