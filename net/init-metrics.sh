...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and NZT_METRICS_CONFIG is
...
host="https://metrics.nexis.network:8086"
...
if $useEnv; then
  [[ -n $NZT_METRICS_CONFIG ]] ||
    usage "NZT_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  NZT_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export NZT_METRICS_CONFIG=\"$NZT_METRICS_CONFIG\"" >> "$configFile"
