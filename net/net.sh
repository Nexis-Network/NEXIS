...
Operate a configured testnet
 ...
 sanity/start-specific options:
   ...
   -o noInstallCheck    - Skipnexis-install sanity
 ...
 update-specific options:
   --platform linux|osx|windows       - Deploy the tarball using 'nexis-install deploy ...' for the
...
build() {
  ...
  if [[ $(uname) != Linux || ! " ${supported[*]} " =~ $(lsb_release -sr) ]]; then
    ...
    source "$NZT_ROOT"/ci/rust-version.sh
...
startCommon() {
  ...
  if $skipSetup; then
    ...
    ssh "${sshOptions[@]}" "$ipAddress" "
      set -x;
      mkdir -p $NZT_HOME/config;
      rm -rf ~/config;
      mv $NZT_HOME/config ~;
      rm -rf $NZT_HOME;
      mkdir -p $NZT_HOME $CARGO_BIN;
      mv ~/config $NZT_HOME/
    "
  else
    # shellcheck disable=SC2029
  ...
  [[ -z "$externalNodeSshKey" ]] || ssh-copy-id -f -i "$externalNodeSshKey" "${sshOptions[@]}" "nexis@$ipAddress"
...
syncScripts() {
  ...
  rsync -vPrc -e "ssh ${sshOptions[*]}" \
    --exclude 'net/log*' \
    "$NZT_ROOT"/{fetch-perf-libs.sh,fetch-spl.sh,scripts,net,multinode-demo} \
    "$ipAddress":"$NZT_HOME"/ > /dev/null
}
...
deployBootstrapValidator() {
  ...
  tar)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$NZT_ROOT"/nexis-release/bin/* "$ipAddress:$CARGO_BIN/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$NZT_ROOT"/nexis-release/version.yml "$ipAddress:~/"
    ;;
  local)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$NZT_ROOT"/farf/bin/* "$ipAddress:$CARGO_BIN/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$NZT_ROOT"/farf/version.yml "$ipAddress:~/"
    ;;
  skip)
    ;;
  *)
...
startBootstrapLeader() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -n "$ipAddress" \
      "./nexis/net/remote/remote-node.sh \
         $deployMethod \
         bootstrap-validator \
         $entrypointIp \
...
startNode() {
  ...
  (
    ...
    if [[ $nodeType = blockstreamer ]] && [[ -n $letsEncryptDomainName ]]; then
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "sudo -H /certbot-restore.sh $letsEncryptDomainName maintainers@nexis.foundation"
...
startClient() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" \
      "./nexis/net/remote/remote-client.sh $deployMethod $entrypointIp \
...
startClients() {
  for ((i=0; i < "$numClients" && i < "$numClientsRequested"; i++)) do
    if [[ $i -lt "$numBenchTpsClients" ]]; then
      startClient "${clientIpList[$i]}" "nexis-bench-tps" "$i"
    else
...
sanity() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$bootstrapLeader" \
      "./nexis/net/remote/remote-sanity.sh $bootstrapLeader $sanityExtraArgs \"$RUST_LOG\""
  ) || ok=false
  $ok || exit 1
  ...
  if [[ -z $skipBlockstreamerSanity && -n $blockstreamer ]]; then
    ...
    (
      ...
      ssh "${sshOptions[@]}" "$blockstreamer" \
        "./nexis/net/remote/remote-sanity.sh $blockstreamer $sanityExtraArgs \"$RUST_LOG\""
    ) || ok=false
    $ok || exit 1
  fi
...
deployUpdate() {
  ...
  for updatePlatform in $updatePlatforms; do
    echo "--- Deployingnexis-install update: $updatePlatform"
    (
      set -x

      scripts/nexis-install-update-manifest-keypair.sh "$updatePlatform"

      timeout 30s scp "${sshOptions[@]}" \
        update_manifest_keypair.json "$bootstrapLeader:nexis/update_manifest_keypair.json"

      # shellcheck disable=SC2029 # remote-deploy-update.sh args are expanded on client side intentionally
      ssh "${sshOptions[@]}" "$bootstrapLeader" \
        "./nexis/net/remote/remote-deploy-update.sh $releaseChannel $updatePlatform"
    ) || ok=false
    $ok || exit 1
  done
}
...
prepareDeploy() {
  ...
  tar)
    if [[ -n $releaseChannel ]]; then
      ...
      rm -f "$NZT_ROOT"/nexis-release.tar.bz2
      declare updateDownloadUrl=https://release.nexis.network/"$releaseChannel"/nexis-release-x86_64-unknown-linux-gnu.tar.bz2
      (
        set -x
        curl -L -I "$updateDownloadUrl"
        curl -L --retry 5 --retry-delay 2 --retry-connrefused \
          -o "$NZT_ROOT"/nexis-release.tar.bz2 "$updateDownloadUrl"
      )
      tarballFilename="$NZT_ROOT"/nexis-release.tar.bz2
    fi
    (
      set -x
      rm -rf "$NZT_ROOT"/nexis-release
      cd "$NZT_ROOT"; tar jfxv "$tarballFilename"
      cat "$NZT_ROOT"/nexis-release/version.yml
    )
    ;;
  local)
    if $doBuild; then
  ...
  *)
  ...
  if [[ -n $deployIfNewer ]]; then
    ...
    if ! diff -q current-version.yml "$NZT_ROOT"/nexis-release/version.yml; then
...
deploy() {
  ...
  if ! $waitForNodeInit; then
    ...
    for ipAddress in "${validatorIpList[@]}" "${blockstreamerIpList[@]}"; do
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "./nexis/net/remote/remote-node-wait-init.sh $((600 - timeWaited))"
    done
  fi
  ...
  tar)
    networkVersion="$(
      (
        set -o pipefail
        grep "^commit: " "$NZT_ROOT"/nexis-release/version.yml | head -n1 | cut -d\  -f2
      ) || echo "tar-unknown"
    )"
    ;;
  local)
...
stopNode() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$ipAddress" "PS4=\"$PS4\" ./nexis/net/remote/cleanup.sh"
  ) >> "$logFile" 2>&1 &
...
logs)
  ...
  fetchRemoteLog() {
    ...
    (
      set -x
      timeout 30s scp "${sshOptions[@]}" \
        "$ipAddress":nexis/"$log".log "$netLogDir"/remote-"$log"-"$ipAddress".log
...
netem)
  if [[ -n $netemConfigFile ]]; then
    ...
    if [[ $netemCommand = "add" ]]; then
      for ipAddress in "${validatorIpList[@]}"; do
        "$here"/scp.sh "$netemConfigFile" nexis@"$ipAddress":"$NZT_HOME"
      done
    fi
    for i in "${!validatorIpList[@]}"; do
      "$here"/ssh.sh nexis@"${validatorIpList[$i]}" 'nexis/scripts/net-shaper.sh' \
      "$netemCommand" ~nexis/nexis/"$remoteNetemConfigFile" "${#validatorIpList[@]}" "$i"
    done
  else
    ...
    for ipAddress in "${validatorIpList[@]}"; do
      "$here"/ssh.sh nexis@"$ipAddress" 'nexis/scripts/netem.sh delete < nexis/netem.cfg || true'
    done

    # Start netem on required nodes
    for ((i=0; i<num_nodes; i++ )); do :
      "$here"/ssh.sh nexis@"${validatorIpList[$i]}" "echo $netemConfig > nexis/netem.cfg; nexis/scripts/netem.sh add \"$netemConfig\""
    done
  fi
  ;;
*)
