# Nexis fork maintenance guide and tools
...
At some point of time we do a "full-update" which merge all solana changes to the nexis develop.
...
In this document we describe some common parts that can be treat differently, and trying to document what changes are done in solana codebase by nexis.

Short algorithm describing how to handle full update:
1. Pointing out what files was changed in nexis since last full-update.
...
4. Ignore files that wasnt modified by nexis (checkout number 1 of this algorithm)
