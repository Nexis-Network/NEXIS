...
nexis_changes="$current_folder/nexis-changes-$BASE-$LOCAL.txt"

remote_changes="$current_folder/nexis-changes-remote-$REMOTE-$LOCAL.txt"
# echo "Forming list of files that differ in current branch from base version $nexis_changes"
...
git diff $LOCAL..$BASE --numstat | cut -f3- | sed -r 's/\{(.*) => (.*)\}/\2/' |  grep -Ev '^(docs|explorer|web3\.js)'  | sort  > $nexis_changes
...
keep_ours_file="$current_folder/nexis-rewrite"

# Nexis use multiple repositories instead of big mono-repo:
# 1. NEXIS is repository only for blockchain related stuff.
# 2. web3-js mooved into https://github.com/Nexis-Network/web3.js
# 3. Explorer mooved into https://github.com/Nexis-Network/native-explorer
# 4. Docs are mooved into https://github.com/Nexis-Network/NEXIS-docs
# 5. account-benches
...
# Filter out changes from file that wasn't touched by nexis changes

for file in $(comm -23 $remote_changes $nexis_changes)
do
...
# sed -e 's/^/\:\^/' $nexis_changes > $pathspec_file
