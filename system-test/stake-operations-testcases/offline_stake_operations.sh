...
if [[ -z "$url" ]]; then
  echo Provide complete URL, ex: "$0" http://api.devnet.nexis.network:8899
  exit 1
fi
nexisconfig set --url $url
...
nexis-keygen new -o "$dummy_keypair" --no-passphrase --force --silent
nexisconfig set --keypair $dummy_keypair
...
nexis-keygen new -o "$online_system_account_keypair" --no-passphrase --force --silent
nexis-keygen new -o "$online_nonce_account_keypair" --no-passphrase --force --silent

online_system_account_pubkey="$(nexis-keygen pubkey $online_system_account_keypair)"
nonce_account_pubkey="$(nexis-keygen pubkey $online_nonce_account_keypair)"
...
nexis-keygen new -o "$offline_system_account_keypair" --no-passphrase --force --silent
nexis-keygen new -o "$offline_staker_keypair" --no-passphrase --force --silent
nexis-keygen new -o "$offline_withdrawer_keypair" --no-passphrase --force --silent
nexis-keygen new -o "$offline_custodian_keypair" --no-passphrase --force --silent

offline_system_account_pubkey="$(nexis-keygen pubkey $offline_system_account_keypair)"
offline_withdrawer_pubkey="$(nexis-keygen pubkey $offline_withdrawer_keypair)"
offline_staker_pubkey="$(nexis-keygen pubkey $offline_staker_keypair)"
offline_custodian_pubkey="$(nexis-keygen pubkey $offline_custodian_keypair)"

# Airdrop some funds to the offline account.
nexisairdrop 100 $offline_system_account_pubkey
nexisairdrop 2 $online_system_account_pubkey

# Create a nonce account funded by the online account, with the authority given to the offline account
nexiscreate-nonce-account $online_nonce_account_keypair 1 --nonce-authority $offline_system_account_pubkey --keypair $online_system_account_keypair
nonce="$(nexisnonce $nonce_account_pubkey)"

execution_step OFFLINE SYSTEM ACCOUNT BALANCE BEFORE CREATING STAKE ACCOUNTS
(
  set -x
  nexisbalance $offline_system_account_pubkey
)
...
nexis-keygen new -o "$stake_account_keypair" --no-passphrase --force --silent
stake_account_address="$(nexis-keygen pubkey $stake_account_keypair)"

sign_only="$(nexiscreate-stake-account $stake_account_keypair 50 \
...
nexiscreate-stake-account $stake_account_keypair 50 \
...
(
  set -x
  nexisstake-account $stake_account_address
)
...
nexis-keygen new -o $split_stake_account_keypair --no-passphrase --force --silent
split_stake_account_address=$(nexis-keygen pubkey $split_stake_account_keypair)

nonce="$(nexisnonce $nonce_account_pubkey)"

sign_only="$(nexissplit-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
nexissplit-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
(
  set -x
  nexisstake-account $split_stake_account_address
)
...
sign_only="$(nexisstake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
  $split_stake_account_address --custodian $offline_custodian_keypair --lockup-epoch 0 \
  --keypair $offline_system_account_keypair --sign-only --url http://0.0.0.0)"

signers="$(get_signers_string "${sign_only[@]}")"

nexisstake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
sign_only="$(nexiswithdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
nexiswithdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
  $split_stake_account_address $offline_system_account_pubkey 10 \
...
vote_account_pubkey="$(awk '{if(NR==4) print $2}'<<<"$(nexisshow-validators)")"
nonce="$(nexisnonce $nonce_account_pubkey)"

# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(nexisdelegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
nexisdelegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(nexisdeactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
# Send the signed transaction on the cluster
nexisdeactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
