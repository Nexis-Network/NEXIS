...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/nexis-install-init nexis-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml nexis-install-init-"$TARGET"* $MAYBE_TARBALLS; do
