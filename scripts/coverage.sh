...
if [[ -z $1 ]]; then
  packages=(--lib --all --excludenexis-local-cluster)
else
  packages=("$@")
fi
...
RUST_LOG=nexis=trace _ "$cargo" nightly test --target-dir target/cov --no-run "${packages[@]}"
if RUST_LOG=nexis=trace _ "$cargo" nightly test --target-dir target/cov "${packages[@]}" 2> target/cov/coverage-stderr.log; then
  test_status=0
else
  test_status=$?
